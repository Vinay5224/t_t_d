 ...
 * tuesdaycollection from MongoDB to the Frontend (DarshanTimeEstimation.jsp) to display the
 * 'From Time' in Tuesday Business Rule Page in Front end for every add rule
 * 
 * @author C.Swapna
 */
@WebServlet("/Createtue")
public class Createtue extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * it's true then it holds the last document in tuesdaycollection from
		 * mongodb and sends the 'ToTime' as a response to the Tuesday
		...
		if (collectioncount != 0) {
			...
			DBCursor cursortue = tuesdaycollection.find().sort(new BasicDBObject("_id", -1)).limit(1);
			 ...
			 * the map.
			 */
			if (cursortue.hasNext()) {

				DBObject results = cursortue.next();
				Map resultmaps = results.toMap();
				ToTime = (String) resultmaps.get("To");
				// Returns ToTime variable to the Tuesday BusinessRule Page(DarshanTimeEstimation.jsp)
